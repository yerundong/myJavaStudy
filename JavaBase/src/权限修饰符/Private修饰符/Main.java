...
@ 作用：有一些变量和方法只作用在本类中，不必被外部访问或子类继承，则可以使用private修饰符；需要访问private成员变量，可通过Getter/Setter方法

@ 可见性：用private修饰的成员，只能类的内部访问，类外部（包括它的子类和父类）来说是隐藏不可见的

@ 修饰对象：
    1.private可以修饰成员变量，成员方法，构造方法，内部类
    2.不能修饰外部类
