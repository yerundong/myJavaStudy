...
/***
@定义： 基本数据类型（除布尔类型外）在某些情况下将发生数据类型自动转换。这些情况包括：运算，传参，赋值

@自动类型转换（提升）特点：
    1.代码不需要进行特殊处理，自动完成。
    2.数据范围从小到大自动转变（与字节数无关），所以也叫提升。
    3.byte/short/char这三种类型在运算的时候，都会被先提升成为int类型，然后计算
    4.只有数字类型和char类型（char类型底层也是储存数字）的数据，也就是与 Number 类相关的数据类型，才可以发生转换（包括自动和强制），
...
public class 基本数据类型之间的自动转换 {
    public static void main(String[] args){
        ...
        // boolean 不能类型转换所以也不能计算
        boolean bool = false;
        // System.out.println(bool == num1);// 编译报错
