...
public class PoetDAOImpl extends BaseDAO<Poet> implements PoetDAO {
     ...
     */
    @Override
    public List<Poet> getPoetByName(Connection cnn, String name) {
        String sql = "SELECT * FROM `jdbc_poet` WHERE `name` = ?";
        List<Poet> poets = query(cnn, sql, name);
        return poets;
    }
}
